...
USING (auth.is_admin() OR auth.is_warden() OR auth.is_security() OR id = auth.uid());
...
CREATE POLICY policy_hostels_admin ON public.hostels FOR ALL TO authenticated USING (auth.is_admin());
...
CREATE POLICY policy_warden_assignments_admin ON public.warden_hostel_assignments FOR ALL TO authenticated USING (auth.is_admin());
...
CREATE POLICY policy_courses_admin ON public.hostel_courses FOR ALL TO authenticated USING (auth.is_admin());
...
USING (
  ...
  OR (auth.is_warden() AND EXISTS (
    ...
    WHERE a.student_id = public.students.id 
      ...
      AND r.hostel_id IN (SELECT auth.get_warden_hostel_ids(auth.uid()))
  ))
);

DROP POLICY IF EXISTS policy_students_admin ON public.students;
CREATE POLICY policy_students_admin ON public.students FOR ALL TO authenticated USING (auth.is_admin());
...
USING (
  ...
  OR (auth.is_warden() AND EXISTS (
    ...
    JOIN public.hostel_rooms r ON b.room_id = r.id
    WHERE b.id = bed_id AND r.hostel_id IN (SELECT auth.get_warden_hostel_ids(auth.uid()))
  ))
);
...
USING (
  auth.is_admin()
  OR (auth.is_warden() AND EXISTS (
    SELECT 1 FROM public.issues i WHERE i.id = issue_id AND i.hostel_id IN (SELECT auth.get_warden_hostel_ids(auth.uid()))
  ))
  OR EXISTS (
...
WITH CHECK (auth.is_admin() OR auth.is_security());
