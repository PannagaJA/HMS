    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        {(['pending', 'approved', 'rejected', 'all'] as const).map((tab) => (
          ...
          >
            {tab} Requests {tab === 'pending' && `(${passes.filter(p => p.status === 'pending').length})`}
          </button>
        ))}
      </div>
            ...
            >
              ...
              {pass.status === 'pending' ? (
                ...
                <div className="text-[11px] text-slate-400 pt-2 border-t border-slate-100">
                  Actioned by: <span className="font-medium text-slate-700">{pass.approved_by_name || 'Warden'}</span>
                  {pass.action_note && <p className="italic mt-0.5">Note: "{pass.action_note}"</p>}
                </div>
              )}
            </div>
          ))
